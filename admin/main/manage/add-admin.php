...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM tb_admin_userdetails WHERE user_email = :email");
        ...
        $userStmt = $pdo->prepare("SELECT user_email FROM tb_admin_userdetails WHERE user_id = :user_id");
        ...
        $logStmt->execute([
            ':doer' => $logEmail,
            ':action' => "Administrator user $user_id added successfully."
        ]);
